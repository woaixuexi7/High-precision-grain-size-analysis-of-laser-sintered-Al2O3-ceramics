...
class RoIHeads(torch.nn.Module):
    ...
    def init_weights(self):
        super(RoIHeads, self).init_weights()
        for m in [self.upsample, self.Conv2, self.Conv4]:
            if m is None:
                continue
    ...
    def forward(self,
        ...
        if self.has_mask():
            ...
            mask_features1 = mask_features
            mask_features1 = self.Conv1(mask_features1)
            mask_features1 = self.relu(mask_features1)
            mask_features2 = self.Conv1(mask_features)
            mask_features2 = self.bn(mask_features2)
            mask_features2 = self.Conv1(mask_features2)
            mask_features2 = self.bn(mask_features2)
            mask_logits1 = self.mask_predictor(mask_features1)
            mask_logits2 = self.Conv2(mask_features2)
            mask_logits3 = self.upsample(mask_logits2)
            mask_concat = torch.cat([mask_logits3,mask_logits1], 1)
            mask_logits = self.Conv4(mask_concat)
